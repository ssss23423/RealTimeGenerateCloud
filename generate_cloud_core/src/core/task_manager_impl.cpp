            ...
            for (auto it = tasks_.begin(); it != tasks_.end();)
            {
                ...
                if (task)
                {
                    if (!task->isFinished())
                    {
                        ...
                        {
                            task->start();
                            Logger::instance().log("Task " + std::to_string(id) + " started.");
                        }
                        ++it;
                    }
                    else
                    {
                        Logger::instance().log("Task " + std::to_string(id) + " finished and removed.");
                        it = tasks_.erase(it);
                    }
                }
                else
...
void TaskManagerImpl::startTask(int id, Task *task)
{
...

    {
        ...
        if (it != tasks_.end())
        {
            Logger::instance().log("Task " + std::to_string(id) + " already exists. Stopping old task.", LogLevel::Warn);
            oldTask = it->second;
            tasks_.erase(it);
        }
    }
    ...
    Logger::instance().log("Task " + std::to_string(id) + " is starting...");
    task->start();
}
...
void TaskManagerImpl::stopTask(int id)
{
    ...
    if (taskToStop)
    {
        Logger::instance().log("Stopping Task " + std::to_string(id));
        taskToStop->stop();
    }
}
